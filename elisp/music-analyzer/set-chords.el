...
(cl-defun sc-root-difference (sc1 sc2)
  (mod (- (sc-root sc2) (sc-root sc1)) 12))


;;; type queries
(cl-defun sc-type-p (sc type)
  "Is SC of type TYPE?"
  (scs-type-p (scs-from-sc sc) type))

(cl-defun sc-tonic-p (sc)
...
;;; relation queries
(cl-defun sc-dominant-fifth-relation-p (sc1 sc2)
  (and (spc-dominant-relation-p (sc-root sc1) (sc-root sc2))
       (sc-dominantic-p sc1)
       (sc-triad-p sc2)))

(cl-defun sc-dominant-dim-relation-p (sc1 sc2)
  "Dm T."
  (and (= 1 (mod (sc-root-difference sc1 sc2) 3))
       (sc-diminished-p sc1)
       (sc-triad-p sc2))) 

(cl-defun sc-dominants-submediant-relation-p (sc1 sc2)
  "Ds T"
  (or
...
(cl-defun sc-dominantic-relation-p (sc1 sc2)
