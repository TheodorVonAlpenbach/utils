...
(cl-defun schord-invert (schord &optional (n 1))
  "Non-destructive."
  (rotate-list schord n))
;;(schord-invert (schord-new) 2)

(cl-defun schord-reduce (schord &optional (ordered t) (trivial nil))
...
;;; Conversions
(cl-defun schord-to-schosk (schord)
  (error "Dangerous")
  (schord-reduce schord t t))

(cl-defun schord-to-schordx (schord)
  (cl-loop with n = (length schord)
	for i below n
...
(cl-defun spc-scale-position (spc key)
...
(cl-defun schord-root-difference (sc1 sc2)
  (mod (- (schord-root sc2) (schord-root sc1)) 12))


;;; type queries
(cl-defun schord-type-p (sc type)
  "Is SC of type TYPE?"
  (scs-type-p (scs-from-sc sc) type))

(cl-defun schord-tonic-p (sc)
...
;;; relation queries
(cl-defun schord-dominant-fifth-relation-p (sc1 sc2)
  (and (spc-dominant-relation-p (schord-root sc1) (schord-root sc2))
       (schord-dominantic-p sc1)
       (schord-triad-p sc2)))

(cl-defun schord-dominant-dim-relation-p (sc1 sc2)
  "Dm T."
  (and (= 1 (mod (schord-root-difference sc1 sc2) 3))
       (schord-diminished-p sc1)
       (schord-triad-p sc2))) 

(cl-defun schord-dominants-submediant-relation-p (sc1 sc2)
  "Ds T"
  (or
...
(cl-defun schord-dominantic-relation-p (sc1 sc2)
