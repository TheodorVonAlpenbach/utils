...
(cl-defstruct (voice :named (:conc-name v-))
  (instrument)
...
;;; constructors etc
(cl-defun notes-copy (notes) (mapcar #'n-copy notes))

;;True copy method of voice.  "
(cl-defun v-copy (voice &key
...
;;; print
(cl-defun instrument-to-string (instrument &optional (print-style mu-default-print-style))
...
(cl-defun v-note-after (voice mposition &optional strictly-after)
...
(cl-defun notes-note-at (notes mposition)
...
(cl-defun v-position (voice mposition &optional strictly-after)
  "Returns the element position of the first note in VOICE starting at or after MPOSITION.
If STRICTLY-AFTER is not nil, the position of the first note
starting after MPOSITION is returned."
...
(cl-defun v-deduce-upbeat (voice &optional (time-signature (v-time-signature voice)))
...
;;; ACTIONS
(cl-defun notes-shift-start-time (notes &optional (shift (notes-start-time notes)))
...
(cl-defun v-shift-start-time (voice &optional (shift (- (v-start-time voice))))
...
Non-destructive."
  (let ((res (cl-loop for n in notes
    ...
    res))
(cl-defun v-section (voice begin-pos &optional (end-pos (v-end-time voice)))
...
(cl-defun notes-delete-in-interval (notes begin-pos end-pos)
...
(cl-defun v-delete-notes-in-interval (voice begin-pos end-pos)
...
(cl-defun notes-remove-in-interval (notes begin-pos end-pos)
...
(cl-defun v-split-at-mposition (voice mposition)
...
TODO: look at the split methods in midi-conversion.el"
  (cl-loop for notes in (notes-split-at-mposition (v-notes voice) mposition)
	collect (v-copy voice :notes notes)))
;;(mapcar #'v-to-string (v-split-at-mposition (v-test) 16))

;; split notes
(cl-defun notes-split-notes (notes d-split-function)
...
(cl-defun v-split-notes (voice d-split-function)
