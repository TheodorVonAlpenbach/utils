...
(cl-defstruct (counterpoint :named (:conc-name cp-))
  (name)
  (style)
  (species) 
  (voices))

(cl-defun cp-new (voices species &optional (style 'grinde)  (name nil))
...
(let ((cp nil)
  ...
  (cl-defun cp-test (&optional (refresh nil) (style 'hindemith) (species 1) (n 1))
...
(cl-defun cp-check (cp)
  (cl-loop for rule in (cp-rules cp)
...
(cl-defun cps-check-print (cps &optional (indent 3))
  (concat* 
   (cl-loop for cp in cps
