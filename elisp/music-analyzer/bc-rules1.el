...
(cl-defun pcs-in-mode (chromees context args)
...
Note that this rule does not consider first and last note. A special rule should cover these cases."
  (cl-loop for pc in (rest (butlast chromees))
	for i from 1
...
;;; General constraints in BC1
(cl-defun n-whole (note context args)
  "All notes should be whole notes."
  (when (/= (n-dtime note) 4)
    "Note a whole note"))

;;; Grinde rules
(cl-defun vis-start (intervals context args)
...
(cl-defun vis-end (intervals context args)
...
(cl-defun vi-dissonance (vinterval context args)
  "Regel 1.2a hos Grinde"
  (unless (or (eq (i-direction vinterval) :down)
	      (i-is vinterval '(P1 m3 M3 P5 m6 M6) #'i~))
    "Dissonance"))

(cl-defun vis-imperfect-consonance-distribution (vintervals context lowest-percentage)
...
Regel 1.2b hos Grinde (en presisering av regel 2a)."
  (let* ((bad-positions (cl-loop for vi in (rest (butlast vintervals 1))
			      for i from 1
...
(cl-defun 2pp-hidden-parallels (vppair1 vppair2 context args)
...
(cl-defun pps-unison (vppairs context args)
...
Regel 1.2d hos Grinde."
  (let ((res (cl-loop for vppair in (rest (butlast vppairs 1))
		   for i from 1
		   if (apply #'p= vppair)
		   collect i)))
    (when res
...
(cl-defun pcp-leading-tone-doubling (vpcpair context args)
...
(cl-defun 2pp-illegal-parallels (vppair1 vppair2 context args)
  "Checks if the vertical pitch pairs VPPAIR1 and VPPAIR2 are illegal parallels.
...
(cl-defun 2p-too-big-interval (pitch1 pitch2 context args)
...
(cl-defun pps-too-long-parallel-sequence (vppairs context max-parallel-sequence-number)
...
(cl-defun hip-too-big-simultaneous-leaps (hipair context maxmin-leap)
...
(cl-defun pp-too-big-interval (vppair context max-interval)
...
(cl-defun pp-stemmekryss (vppair context args)
...
(cl-defun 2p-repetition (pitch1 pitch2 context args)
