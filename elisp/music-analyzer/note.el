...
(cl-defstruct (note :named (:conc-name n-))
...
(cl-defun n-new (&optional (pitch (p-new)) (start-time 0.0) (duration (d-new 1)) (tied nil))
  (make-note :pitch pitch :start-time start-time :duration duration :tied tied))
;;(n-new (p-new 1) 1.5 (d-new 2) t)

(cl-defun n-copy (note &key
		     (pitch (n-pitch note))
...
(cl-defun n-alteration-p (note1 note2 &optional n)
...
(cl-defun n-split-across-bars (note time-signature)
...
(cl-defun n-split-1 (note d-split)
  (let ((res (cl-loop with st-orig = (n-start-time note)
...
(cl-defun n-split (note d-split-function)
  "Splits a note that crosses a measure bar"
...
(cl-defun n-from-string-lilypond (note-string)
