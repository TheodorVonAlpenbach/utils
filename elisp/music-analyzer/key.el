...
(cl-defstruct (key :named (:conc-name k-))
  (root (make-chrome))
  (mode 'major))

(cl-defun k-new (&optional (root (make-chrome)) (mode 'major))
  (make-key :root root :mode mode))
;;(k-to-string (k-new))

(cl-defun k-scale (key &optional (with-alterations t))
...
(cl-defun k-from-strings (chrome-string mode-string &optional (print-style mu-default-print-style))
  (make-key :root (chrome-from-string chrome-string print-style)
	    :mode (mode-from-string mode-string print-style)))

(cl-defun k-from-string (key-string &optional (print-style mu-default-print-style))
