...
(cl-defstruct (cp-context :named (:conc-name cchrome-))
  key
  time-signature
  super-p
  voices
...
(cl-defun cchrome-new (key time-signature super-p)
...
(cl-defun apply-2pitches-rule (voices rule context args)
  ""
  (cl-loop for v in voices
	append (cl-loop for hnp in (v-notes v)
		     for hpp in (mapcar #'note-pair-to-pitch-pair hnp)
		     for res = (funcall rule (first hpp) (second hpp) context args)
		     if res collect (format "In voice '%s', note %s: %s"
				      (v-instrument v) (n-to-string (first hnp)) res))))

(cl-defun apply-pitch-pairs-rule (voices rule context args)
  ""
  (cl-loop for vp in (relations voices)
...
(cl-defun apply-2pitch-pairs-rule (voices rule context args)
  ""
  (cl-loop for vp in (relations voices)
	append (cl-loop for 2np in (pairs (apply #'2voices-to-note-pairs vp))
...
(cl-defun apply-chrome-rule (voices rule context args)
  ""
  (cl-loop for v in voices
	append (cl-loop for n in (v-notes v)
...
(cl-defun apply-chrome-pairs-rule (voices rule context args)
  ""
  (cl-loop for vp in (relations voices)
...
(cl-defun apply-chrome-pair-rule (voices rule context args)
  ""
  (cl-loop for vp in (relations voices)
	for pcps = (mapcar #'note-pair-to-chrome-pair (apply #'2voices-to-note-pairs vp))
	append (cl-loop for pcp in pcps
		     for res = (funcall rule pcp context args)
		     if res collect (format "In voices %S: %s" (mapcar #'v-instrument vp) res))))
      
(cl-defun apply-vertical-intervals-rule (voices rule context args)
...
(cl-defun apply-vertical-interval-rule (voices rule context args)
  "Works only in 1st specices counterpoint"
  (cl-loop for vp in (relations voices)
	for vis = (apply #'2voices-to-vertical-intervals vp)
	append (cl-loop for vi in vis
		     for i from 1
		     for res = (funcall rule vi context args)
		     if res collect (format "In voices %S at position %d: %s" 
				      (mapcar #'v-instrument vp) i res))))

(cl-defun apply-horizontal-interval-pair-rule (voices rule context args)
  (cl-loop for vp in (relations voices)
	append (cl-loop for hi1 in (voice-to-horizontal-intervals (first vp))
...
(cl-defun rule-type (rule)
  (first (split-string (symbol-name rule) "-")))
;;(rule-type #'2notes-repetition)

(cl-defun vs-apply-rule (voices rule context)
...
(cl-defun vs-apply-rules (voices rules context)
...
(cl-defun vgs-apply-rules (voice-groups rules)
  (cl-loop for vg in voice-groups
	collect (cons (vg-name vg) (vg-apply-rules vg rules))))
;;(vgs-apply-rules (test-vgs nil) b1-rules)

(cl-defun mvt-apply-rules (movement rules)
