...
(cl-defun lp-time (time-signature)
  (format "%s %d/%d" 
    (lp-variable 'time)
    (ts-length time-signature)
    (ts-unit time-signature)))

(cl-defun lp-clef (clef)
  (format "%s %S" (lp-variable 'clef) clef))

(cl-defun lp-relative (absolute-pitch music)
  (format "%s = %s {\n%s\n}" (lp-variable 'relative) (p-to-string pitch) music))

(cl-defun lp-define-variable (name value)
  (format "%s = %s \n" name (if (listp value) (lp-list value) value)))

(cl-defun lp-list (list)
  (concat* list :pre "{ " :in " " :suf " }"))
;;(lp-list '())

(cl-defun lp-simultanous-music (&rest musics)
  (concat*  musics :pre "<< \n" :in " " :suf ">> \n"))

(cl-defun lp-simultanous-music* (musics)
  (concat*  musics :pre "<< \n" :in "\n\\\\\n" :suf ">> \n"))

(cl-defun lp-sequential-music (&rest musics)
  (concat* musics :pre "{ \n" :in " " :suf "\n}"))

(cl-defun lp-staff-group (&rest staffs)
  (format "\\new StaffGroup %s" (apply #'lp-simultanous-music staffs)))

(cl-defun lp-choir-staff (&rest staffs)
  (format "\\new ChoirStaff %s" (apply #'lp-simultanous-music staffs)))

(cl-defun lp-staff (music &optional name)
  (format "\\new Staff %s" music))

(cl-defun lp-bar (type)
  (format "%s \"%s\"" (lp-variable 'bar) type))

(cl-defun lp-voice (music &optional name)
  (if name 
    (format "\\new Voice = \"%s\" { \n %s \n } \n" name music)
    (format "\\new Voice %s" music)))
;;(insert (lp-voice "qwe" "qwe"))

(cl-defun lp-upper-voice-context (&rest musics)
  (format "\\voiceOne %s" (concat*  musics :in " ")))

(cl-defun lp-lower-voice-context (&rest musics)
  (format "\\voiceTwo %s" (concat*  musics :in " ")))

(cl-defun lp-score (score-body)
  (format "%s {\n %s }" (lp-variable 'score) score-body))
;;(lp-score "a")

(cl-defun lp-upbeat (upbeat)
  (when upbeat (format "%s %s" (lp-variable 'partial) (d-to-string upbeat 'lilypond))))
;;(lp-upbeat 1.0)

(cl-defun collect-globals (movement)
...
(cl-defun mvt-lp-voice (mvt n &optional (with-repeats t))
...
(cl-defun v-to-lp-voice (voice repeats &optional (clef 'treble))
  "Converts the Nth voice of MVT to lilyond format text, including repeat patterns if WITH-REPEATS is not nil.
Assumes only one repetition pattern in mvt"
  (lp-sequential-music
   (lp-clef clef)
...
(cl-defun vg-to-lp-choir-staff (vg global repeats)
  (lp-choir-staff 
...
(cl-defun movement-to-lilypond-string (movement title &optional (style 'lp-choral-style))
...
Currently only LP-CHORAL-STYLE style is supported"
  (let ((mu-default-print-style 'lilypond))
    ...
    (let* ((title-line (and title (lp-markup (lp-line (format "{%s}" title)))))
	   ...
	   (staff-groups (cl-loop for vg in vgs
...
(cl-defun movement-to-lilypond-string-old (movement title &optional (style 'lp-choral-style))
  "Converts MU MOVEMENT to lilypond format and writes result to FILE.
Currently only LP-CHORAL-STYLE style is supported"
  (let ((mu-default-print-style 'lilypond))
    (unless (eq style 'lp-choral-style)
...
(cl-defun movement-to-lilypond (movement &key 
				       (title nil) 
