...
(let ((hcp nil)
  ...
  (cl-defun hcp-test (&optional (refresh nil) (name nil) (file nil))
    (when file
      (setq hcp-file file)
      (setq refresh t))
...
(cl-defun hcp-model-rules (cp)
  (cl-loop for i from 1 to 13 nconc
	(funcall (intern (format "hcp-rule-%d" i))
		 (hcp-model cp))))

(cl-defun hcp-two-part-rules (cp)
  (nconc
   (hcp-model-rules cp)
   (cl-loop for i from 14 to 28 nconc
	 (funcall (intern (format "hcp-rule-%d" i)) cp))))

(cl-defun hcp-rule-1 (voice)
...
I.e. only three consecutive melodic intervals in the same direction is accepted."
  (cl-loop for g in (group (pairs (v-notes voice)) 
...
(cl-defun hcp-rule-5 (voice &optional (model-p t))
...
(cl-defun hcp-rule-6 (voice)
  "No tone-repetition."
  (nconc 
   (cl-loop for g in (group (v-notes voice) :test #'p= :key #'n-pitch)
	 if (> (length g) 1)
	 collect (format "Rule 6 violation: Note repetition after note %s"
		   (n-to-string (first g))))
   ;; auxilliary notes
   (let* ((n-zip (zip (v-notes voice))))
     (cl-loop for n in (sort*  (nconc (cl-loop for g in (group (first n-zip) :test #'p= :key #'n-pitch)
					 if (> (length g) 1) collect (first g))
				   (cl-loop for g in (group (second n-zip) :test #'p= :key #'n-pitch)
					 if (> (length g) 1) collect (first g)))
			    #'< :key #'n-start-time)
	   collect (format "Rule 6 warning: 'note - auxillary note - note' formation from note %s" (n-to-string n))))))

(require 'set-chords)
(cl-defun hcp-illegal-melodic-triad-p (triad &optional (model-p t))
...
(cl-defun hcp-rule-7 (voice &optional (model-p t))
  "No broken chords."
  (nconc 
   (cl-loop for n-triad in (tuples (v-notes voice) 3)
   ...
   (cl-loop for 4n in (tuples (v-notes voice) 4)
...
(cl-defun hcp-rule-8 (voice)
  "No sequences."
  (cl-loop for l from 3 to (/ (v-length voice) 2) 
	nconc 
	(cl-loop for i below l
...
(cl-defun hcp-rule-9 (voice &optional (model-p t))
...
(cl-defun hcp-rule-10 (voice &optional (model-p t))
  "No more than one skip in the same direction."
  (cl-loop for 22n in (pairs (pairs (v-notes voice)))
...
(cl-defun hcp-rule-13 (voice)
  ...
  (nconc 
   ...
   (cl-loop for 3n in (tuples (v-notes voice) 3)
...
(cl-defun hcp-rule-16 (cp)
  "No crossing of voices. Assumes two voices."
  (cl-loop for 22hn in (pairs (cp-chords cp))
...
(cl-defun hcp-rule-17 (cp)
  "Pitch-distance determined by the voices used"
  nil)

(cl-defun hcp-rule-18 (cp)
...
(cl-defun hcp-not-root-intervals-at (hcp notes)
...
are issued from this rule."
  (nconc
   (cl-loop for ch in (hcp-not-root-intervals-at cp (v-pitch-minimum-notes (hcp-model cp)))
   ...
   (cl-loop for ch in (hcp-not-root-intervals-at cp (v-pitch-maximum-notes (hcp-model cp)))
	 collect (format "Rule 19 warning: Interval %s at a highest pitch point %s in model is not in root position"
		   (i-abbrevation (i-new ch)) (p-to-string (n-pitch (first ch)))))
   (cl-loop for ch in (hcp-not-root-intervals-at cp (v-pitch-minimum-notes (hcp-2nd cp)))
...
(cl-defun i-tritone-p (interval)
  (i-is interval '(A4 d5) #'i~))

(cl-defun i-augmented-p (interval)
  (i-is interval '(A1 A2 A3 A4 A5 A6 A7) #'i~))

(cl-defun hcp-rule-21-base (22vn sc-typename)
  (let* ((2vn1 (first 22vn))
	 (2vn2 (second 22vn)))
    (when sc-typename
      (nconc
...
(cl-defun hcp-rule-22 (cp)
  "No simultaneous leaps in the same direction"
  (cl-loop for 22vn in (copy-if #'(lambda (2hi) (and (apply #'all-true (mapcar #'i-skip-p 2hi))
							(apply #'eq (mapcar #'i-direction 2hi))))
			     (pairs (cp-chords cp)) 
...
(cl-defun voices-relations-to-other-voices-indexes (voices)
  (relations (0-n (length voices)) nil t))
;;(voices-relations-to-other-voices-indexes '((s1 s1) (a1 a2) (t1 t2) (b1 b2)))

(cl-defun voices-relations-to-other-voices-notes (voices)
  (voices-relations-to-other-voices-indexes))

(cl-defun hcp-cross-relation (chord1 chord2 format-string)
...
(cl-defun hcp-rule-23 (cp)
  "No cross-relations"
  (nconc (cl-loop for 2ch in (pairs (cp-chords cp)) nconc
	       (hcp-cross-relation (first 2ch) (second 2ch) "Rule 23 violation: Cross relation between note %s in voice %d and note %s in voice %d"))
	 (cl-loop for 2ch in (remove-nth 1 (tuples (cp-chords cp) 3)) nconc
...
(cl-defun hcp-rule-24 (cp)
  "No unison, octave, fifth or fourth parallels"
  (cl-loop for 22vn in (pairs (cp-chords cp))
...
(cl-defun hcp-rule-26 (cp)
  "No covered unisons or octaves"
  (cl-loop for 22vn in (copy-if #'(lambda (22vn)
...
(cl-defun hcp-rule-28 (cp)
  ...
  (let ((high1 (v-pitch-maximum-notes (hcp-model cp)))
    ...
    (nconc 
     (cl-loop for n in (intersection high1 high2 :key #'n-start-time)
     ...
     (cl-loop for n in (intersection low1 high2 :key #'n-start-time)
     ...
     (cl-loop for n in (intersection high1 low2 :key #'n-start-time)
     ...
     (cl-loop for n in (intersection low1 low2 :key #'n-start-time)
