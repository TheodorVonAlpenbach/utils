...
(cl-defun d-time (duration)
  "Convert to an absolute time."
  (d-value duration))

(cl-defun d-add (duration1 duration2)
...
(cl-defun dvalue-split-at-constant (dvalue max-length)
...
(cl-defun d-split-at-constant (duration max-length)
  "Helper function for dvalue-split-illegal-duration"
  (dvalue-split-at-constant (d-value duration) max-length))
;;(mapcar (bind #'d-split-at-constant 4) (mapcar #'d-new '(1 2 3 4 5 6 7 8 9 )))

(cl-defun dvalue-split-illegal-duration (dvalue legal-durations-values)
...
(cl-defun d-split-illegal-duration (duration legal-durations-values)
...
(cl-defun d-split-total (duration unit)
...
;;; read/write
(cl-defun d-to-string-lilypond (duration)
...
(cl-defun d-from-string-lilypond (duration-string &optional (print-style mu-default-print-style))
