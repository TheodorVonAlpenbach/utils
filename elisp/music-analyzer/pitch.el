...
(cl-defstruct (pitch :named (:conc-name p-))
  (chrome (make-chrome))
  (octave 4))
;;(p-chrome (make-pitch) 'lilypond)
;;(p-to-string (make-pitch) 'lilypond)

(cl-defun p-new (&optional (chrome (chrome-new)) (octave 4))
  (make-pitch :chrome chrome :octave octave))
;;(p-new)

(cl-defun p-copy (pitch &key 
...
(cl-defun p-set-chrome (pitch)
  (spc-to-chrome (p-chrome pitch)))

(cl-defun p-ansi (pitch &optional (base-octave 0))
...
(cl-defun p< (pitch1 pitch2)
  (< (p-ansi pitch1) (p-ansi pitch2)))
;;(p< (p-new) (p-new (chrome-new 1)))

(cl-defun p> (pitch1 pitch2) (p< pitch2 pitch1))
(cl-defun p<= (pitch1 pitch2) (not (p< pitch2 pitch1)))
(cl-defun p>= (pitch1 pitch2) (p<= pitch2 pitch1))
(cl-defun p= (pitch1 pitch2) (equal pitch1 pitch2))
(cl-defun p/= (pitch1 pitch2) (not (p= pitch1 pitch2)))
;;(p= (p-new) (p-new (chrome-new 0)))

(cl-defun p+ (pitch1 pitch2 &optional (n 1))
...
(cl-defun p-transpose (pitch interval &optional (n 1))
  (p+ pitch interval n))
...
(cl-defun p-interval (pitch1 pitch2)
  (p- pitch2 pitch1))
;;(p-interval (p-from-string "B2") (p-from-string "F4"))

(cl-defun p-nalterate (pitch alteration)
  "See `chrome-nalterate'"
  (chrome-nalterate (p-chrome pitch) n))

(cl-defun p-alterate (pitch n)
  "See `chrome-alterate'"
  (p-nalterate (p-copy pitch) n))

(cl-defun p-alteration-p (pitch1 pitch2 &optional n)
...
(cl-defun p2-to-string (pitch-pair &optional (print-style mu-default-print-style))
  (concat* (mapcar (bind #'p-to-string print-style) pitch-pair) :pre "[" :in " " :suf "]"))

(cl-defun p-from-string-lilypond (p-string)
...
(cl-defun p-from-string (p-string &optional (print-style mu-default-print-style))
