...
(defun start-socket-server (passive-socket &key function wait)
  ...
  (unwind-protect
    (loop (let ((connection  (ext:socket-accept passive-socket)))
		...
		(handler-case (values-list (cons t (multiple-value-list
						    (close connection))))
...
(defvar *close-apache-socket* t) ;set to t if you want to close the socket to apache
...
(defun process-apache-command (command)
  (let* ((url (cdr (assoc "url" command :test #'string=)))
	 (html (if (equal (cdr (assoc "url" command :test #'string=))
			  "/foobar/fixed")
		 (fixed-html)
;;		(debug-table command))))
		 (fixed-html2 url))))
...
<TR bgcolor=\"yellow\"><TH COLSPAN=2>CLISP + mod_lisp 2.0 + apache + Cygwin</TH></TR>
...
<HTML><HEAD></HEAD><BODY><H1>mod_lisp 2.0</H1><P>This is a constant
  html string sent by mod_lisp 2.0 + CLISP + apache + Cygwin</P>
</BODY></HTML>")

(defun fixed-html2 (url)
  (format nil "<!DOCTYPE HTML PUBLIC \"-//IETF//DTD HTML//EN\">
<HTML><HEAD></HEAD><BODY><H1>mod_lisp 2.0</H1><P>This is a constant
  html string sent by mod_lisp 2.0 + CLISP + apache + Cygwin on URL ~a</P>
</BODY></HTML>" url))
