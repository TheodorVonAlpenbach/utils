...
`directory-file-name', file-name-all-completions, file-name-completion"
  (cond 
   ((eq operation 'write-region) (apply #'ftp-write-region args))
   ...
   (t 
;    (message "Primitive `(%s %s)'" operation (concat* args :in " " :key #'(lambda (x) (format "%S" x))))
    (ftp-run-real-handler operation args))))

(defun ftp-write-region (start end url &optional append visit lockname mustbenew)
  "TODO: handle append and rest of options"
  (when (or append lockname mustbenew) 
    (error "ftp-write-region doesn't support optional parameters"))
  (let ((string (buffer-substring start end))
	(temp-filename (make-temp-file "ftp")))
    (write-region start end temp-filename)
    (ftp-put (ftp-process url) temp-filename (ftp-remote-path url))))
...
(defun ftp-insert-file-contents (url &optional visit beg end replace)
  (let ((temp-filename (make-temp-file "ftp")))
    (ftp-get (ftp-process url) (ftp-remote-path url) temp-filename)
    (let ((buffer-file-name-exists-p buffer-file-name)
	  (bytes (second (insert-file-contents temp-filename visit beg end replace))))
