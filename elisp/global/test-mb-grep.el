...
mb-grep-dirs."
  (let* ((dir (expand-directory-name +mb-lisp-dir+))
    ...
    (should (find pdir (mb-grep-dirs 1 dir) :test #'string=))
    (should-not (find "/" (mb-grep-dirs 1 dir) :test #'string=))
    (should (equal (mb-grep-dirs 10 dir)
		   (list (expand-directory-name ".." dir))))
    (should (find pdir (mb-grep-dirs 11 dir) :test #'string=))
    (should (find dir (mb-grep-dirs 11 dir) :test #'string=))
    ;; minus
    (should (equal (mb-grep-dirs "-" dir) (list dir)))
    (should (find dir (mb-grep-dirs -1 dir) :test #'string=))
    (should (find (expand-directory-name "~/.emacs.d/")
