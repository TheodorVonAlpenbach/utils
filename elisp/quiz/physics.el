...
(cl-defun circle-area (radius)
  (* pi (sq radius)))

(cl-defun circle-circumference (radius)
  (* pi radius))

(cl-defun sphere-volume (radius)
  (* (/ 4.0 3) pi (expt radius 3)))
;;(sphere-volume +earth-radius+)

(cl-defun mass-from-volume (volume density)
  (* volume density))
;;(mass-from-volume (sphere-volume +earth-radius+) +earth-density+)

(cl-defun mass-density (mass volume)
  (/ (float mass) volume))

(cl-defun surface-gravity (&key (radius +earth-radius+) (density +earth-density+) (mass (mass-from-volume (sphere-volume radius) density)))
  (/ (* +gravitational-constant+ mass)
     (sq radius)))
;;(surface-gravity)

(cl-defun time-to-touchdown (height)
...
(cl-defun velocity-from-height (height &optional  (velocity-unit '(:meter :second)))
...
(cl-defun velocity-from-kinetic-energy (kinetic-energy mass &optional  (velocity-unit '(:meter :second)))
...
(cl-defun acceleration-st (distance time)
  (/ (* 2 (float distance)) (sq time)))
;;(acceleration-st 70 3.8)

(cl-defun radius-from-gravity (gravity))

(cl-defun revolution-speed (distance-from-center &optional (planet-mass +earth-mass+))
  "P 132."
  (sqrt (/ (* +gravitational-constant+ planet-mass) distance-from-center)))
;;(revolution-speed (+ 1E6 +earth-radius+))

(cl-defun period-of-revolution (distance-from-center &key (planet-mass +earth-mass+)
...
(cl-defun moment-of-inertia-hoop (mass radius)
  "P 254"
  (* mass (sq radius)))
;;(moment-of-inertia-hoop 0.030 (/ 0.065 2))3.1687500000000005e-05

(cl-defun moment-of-inertia-cylinder (mass radius)
  (* .5 mass (sq radius)))

(cl-defun moment-of-inertia-hollow-cylinder (mass outer-radius inner-radius)
  (* .5 mass (+ (sq outer-radius) (sq inner-radius))))
;;(moment-of-inertia-hollow-cylinder 0.030 0.0325 0.0324)

(cl-defun moment-of-inertia-sphere (mass radius)
  (* 0.4 mass (sq radius)))

(cl-defun angular-velocity-from-period (period-of-revolution)
  "P 246, 250"
  (/ (* 2 pi) period-of-revolution))

(cl-defun period-from-angular-velocity (angular-velocity)
  "I.e. constant angular-velocity"
  (/ (* 2 pi) angular-velocity))
;;(period-from-angular-velocity (angular-velocity-from-period 1))

(cl-defun rpm-to-period (rpm)
  (change-time-unit (/ 1.0 rpm) :from-unit :minute))
;;(* (rpm-to-period 33) 33)

(cl-defun angular-velocity-from-rpm (rpm)
  (angular-velocity-from-period (rpm-to-period rpm)))
;;(angular-velocity-from-rpm 33)

(cl-defun angular-velocity-of-car-wheel (wheel-radius car-speed)
...
(cl-defun potential-energy (height mass)
  (* mass +gravity+ height))

(cl-defun kinetic-energy (mass velocity)
  (* 0.5 moment-of-inertia angular-velocity))

(cl-defun angular-kinetic-energy (moment-of-inertia angular-velocity)
  (kinetic-energy moment-of-inertia angular-velocity))

(cl-defun velocity-from-sphere-rolling-down (mass radius height rotation-period)
...
(cl-defun how-many? (x nx y ny z nz thing)
...
(cl-defun selection-with-repetition (number-of-distinct-objects repetitions)
