...
(cl-defun num-in-args (expression)
  (cl-destructuring-bind (f s) expression
    (unless (eql (caar f) '_id\.userPseudonym)
      (rotatef f s))
     (assert (eql (caar s) '_id\.componentUuid))
...
(cl-defun qwe (mongo-query)
  (awhen (condition-case nil
	...
	(coerce (cdr (cadr (second it))) 'list)
	(coerce (cdr (cadr (second it))) 'list)))))
...
(cl-defun dd-parse-query (mongo-query)
  (awhen (condition-case nil
    ...
    (when (and (string= (cdr (first it)) "userComponentState")
	       (eql (car (third it)) 'projection)
	       (eql (car (second it)) 'filter))
      (case (caadr (second it))
	($and (num-in-args (coerce (cdr (cadr (second it))) 'list)))
	($or (list 1 (length (coerce (cdr (cadr (second it))) 'list))))
	(_id (list 1 1))
...
;; (concat* (sort (remove-duplicates (cl-loop for x in dd-table collect (caar x))) #'<) :in " " :key #'sstring)
;; (concat* (sort (remove-duplicates (cl-loop for x in dd-table collect (cadar x))) #'<) :in " " :key #'sstring)
