...
(cl-defun ada-curriculum-p (x)
  (and (listp x)
       (> (length x) 10)
       (cl-notany #'listp x)))
;;(ada-curriculum-p (curriculum-from-id 321211))

(cl-defun curriculum-from-name (name &rest columns)
...
(cl-defun curriculum-from-curriculum-name (curriculum-name &rest columns)
...
(cl-defun curriculum-id-from-pseudonym (curriculum-pseudonym)
  (caar (emacsql db
...
(cl-defun curriculum-from-pseudonym (curriculum-pseudonym &rest columns)
  (car (emacsql db
	 (vector :select (column-selection columns)
		 :from 'curriculum
		 :where '(= id $s1))
...
(cl-defun curriculum-from-id (curriculum-id-descriptor &rest columns)
  (car (emacsql db
	 (vector :select (column-selection columns)
		 :from 'curriculum
		 :where '(= id $s1))
...
(cl-defun curriculum (curriculum-descriptor &rest columns)
...
(cl-defun fcurriculum (curriculum-descriptor &rest columns)
  "Arugments COLUMNS are not yet supported"
  (tab-format (butlast (cl-loop for v in (curriculum curriculum-descriptor)
