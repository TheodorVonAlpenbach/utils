...
(defun task (task &rest columns)
  (apply #'task-from-id task columns))
;;()

(require 'ada-element)
(defun element-tasks (element &rest columns)
  (emacsql db
	 (vector :select (column-selection columns)
		 :from 'task
		 :where '(= element-id $s1))
...
(defun component-tasks (component &rest columns)
  (loop for x in (component-element-ids component) 
