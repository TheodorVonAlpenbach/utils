...
(defun card-value (n) (mod n 13))
(defun card-color (n) (/ n 13))
(defun card-red-p (n) (< 0 (card-color n) 3))
(defun card-black-p (n) (not (card-red-p n)))

(defun card-ace-p (n) (zerop (card-value n)))
(defun card-king-p (n) (= (card-value n) 12))

(defun card-parse-value (char)
  (case (upcase char)
...
(defun card-parse (string-or-symbol &optional split-p)
