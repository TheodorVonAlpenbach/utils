...
(cl-defun chess-board-face (white-p)
  (if white-p
    chess-board-white-face
    chess-board-black-face))

(cl-defun chess-board-canvas-size (direction)
...
(cl-defun chess-board-canvas-make (&optional (char 32))
...
(cl-defun chess-board-canvas-make (&optional (char 32))
  ...
  (let* ((s (make-string (rowwise chess-board-square-size) char))
	 ...
	 (even-line (concat* (cl-loop for i below 4 collect dark-square-line collect light-square-line)))
	 (odd-line (concat* (cl-loop for i below 4 collect light-square-line collect dark-square-line)))
	 (lines (cl-loop with height = (columnwise chess-board-square-size)
		      for i below 8
		      append (make-list 3 (if (evenp i) even-line odd-line)))))
    (concat* (nreverse lines) :in "\n" :suf "\n")))

(cl-defun chess-board-canvas-insert ()
  (insert (chess-board-canvas-make)))


;;; Conversions
(cl-defun esquare (square)
  (list (first square) (- 7 (second square))))

(cl-defun edge-start-position (offset n edge-length)
  (+ offset (* n edge-length)))

(cl-defun edge-mid (edge-length) (/ edge-length 2))
(cl-defun square-mid () (mapcar #'edge-mid chess-board-square-size))
;;(square-mid)

(cl-defun chess-board-start-position ()
...
;;; Other queries
(cl-defun chess-board-square-dark-p (square-description)
  (evenp (snumber square-description)))

(cl-defun chess-board-insert-colored-string (string square face &optional (start-pos 1))
...
(cl-defun chess-board-insert-piece (piece &optional (start-pos 1))
...
(cl-defun chess-board-print (pieces &optional (buffer "*chess-print*"))
  (switch-to-buffer buffer)
  (fundamental-mode)
  (chess-board-canvas-insert)
  (cl-loop for p in pieces do
