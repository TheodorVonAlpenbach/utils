...
(cl-defun square-to-snumber (square)
  (+ (* (second square) 8)
     (first square)))
;;(square (square-to-snumber '(0 1)))

(cl-defun square-reflect-y (square)
  "Reflects by 0 row"
  (list (- (first square)) (second square)))

(cl-defun square-reflect-x (square)
...
(cl-defun square-outside-board-p (square)
  (or (< (first square) 0)
      (< (second square) 0)
      (> (first square) 7)
      (> (second square) 7)))
...
(cl-defun snumber (square-description)
  (square-to-snumber (square square-description)))
;;(mapcar #'snumber '("A1" "B1" "A2" "B2" "C3" "D4" "E5" "H8"))

(cl-defun snumber-to-row-number (snumber)
  (/ snumber 8))
(cl-defun snumber-to-column-number (snumber)
  (mod snumber 8))

(cl-defun snumber-on-same-column-p (snumber1 snumber2)
  (zerop (mod (- snumber1 snumber2) 8)))
;;(snumber-on-same-column-p 8 17)

(cl-defun snumber-on-ne-diagonal (row-number)
  (nth row-number ne-main-diagonal))

;;; sname ie. a1, b2 etc
(cl-defun sname (square-description)
...
(cl-defun squares (&optional (dimensions '(8 8)))
  (cl-loop for row below (square-row dimensions)
	collect (cl-loop for column below (square-column dimensions)
		     collect (list column row))))
;;(squares '(3 4))

(cl-defun square-invert (square &optional (rows 8))
  "Inverts square"
...
(cl-defun square-light-p (square &optional (first-square-is-dark-p t))
