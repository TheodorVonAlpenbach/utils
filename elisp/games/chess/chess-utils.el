(cl-defun symbol-to-char (symbol)
  (char (symbol-name symbol) 0))

(cl-defun chars-to-string (chars)
  (apply #'string chars))

(cl-defun string* (chars &key (in nil))
  (let ((chars* (cl-loop for char in chars
		      for i from 0
		      collect char
...
(cl-defun count-lines-buffer (&optional (buffer (current-buffer)))
  "Returns the number of lines in BUFFER"
  (with-buffer buffer
    (count-lines (point-min) (point-max))))
;;(count-lines-buffer)

(cl-defun line-length (&optional (line (line-number-at-pos)) (buffer (current-buffer)))
...
(cl-defun negative-p (number &optional (strictly-p t)) 
...
(cl-defun positive-p (number &optional (strictly-p t)) 
...
;;(column-line 1923)
(cl-defun ecl-line (column-line) (second column-line))
...
(cl-defun overwrite (printable)
  "Overwrites at point with PRINTABLE"
  (let ((s (sstring printable)))
    (insert s)
    (kill-region (point) (+ (point) (length s)))))

(cl-defun insert-at (printable column-line &optional overwrite-p (buffer (current-buffer)))
