...
(cl-defstruct (chess-board-square
	    (:conc-name cbs-))
...
(cl-defun cbs-new (&optional (symbol "") dark-p)
...
(cl-defun cbs-clone (cbs &key size address color symbol symbol-alignment)
...
(cl-defun cbs-insert-at (cbs column-line &optional overwrite-p)
  ...
  (let ((ecl (ecl-add column-line (cbs-position cbs))))
    (cl-loop for i from (ecl-line ecl) below (+ (ecl-line ecl) (cbs-height cbs))
...
(cl-defun cbs-insert (cbs &optional overwrite-p (point (point)))
...
(cl-defstruct (chess-board-canvas
	    (:conc-name cbc-))
  (squares))
;;(make-chess-board-canvas)

(cl-defun cbc-new (&optional (square-size cbs-default-size) 
...
(cl-defun cbc-insert-at (cbc column-line &optional reversed-p)
  "Inserts a CBS at point"
...
(cl-defun cbc-insert (cbc &optional (point (point)))
...
(cl-defstruct (chess-board-border
	    (:conc-name cbb-))
  (canvas) ;;an (1 n) or (n 1) CBS
  (margin-squares (list nil nil))
  )

(cl-defun cbb-new (symbols cbs-template margins orientation)
...
ORIENTATION is either :vertical or :horizontal"
  (let* ((cbss (list (cl-loop for s in symbols
			   for i from 0
