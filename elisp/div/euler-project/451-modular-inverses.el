...
(cl-defun modular-inverse-p (i n) (= (mod (* i i) n) 1))
;;(modular-inverse-p 11 15)

(cl-defun find-max-modular-inverse (n)
  (cl-loop for i from (- n 2) downto 1
...
(cl-defun find-min-modular-inverse (n)
  (or (cl-loop for i from 2 to (floor (/ n 2))
	    if (= (mod (* i i) n) 1) return i)
      1))
...
;;(cl-loop for i from 10000 to 100000 if (primep i) return i)
;;(primep 104743)
;;(aref +fvec+ 104743)
;;(aref +fvec+ 100007)
;;(aref +fvec+ 11)

(cl-defun cl-make-factor-vector (n primes)
  (let ((res (make-array (list n 2) :initial-element 1)))
    (cl-loop for i below n do (setf (aref res i 0) i))
...
;(time (cl-loop for n from 2 below 1000000 do (factorize-rec n +fvec+)))
;;(position nil +fvec+ :start 2)

(cl-defun cl-factorize-rec (n fvec)
...
(cl-defun factorize-all (n primes)
...
(cl-defun find-min-modular-inverses (n)
  (cl-loop with factors = (factorize-all n 10000-first-primes)
...
(cl-defun all-factors-3 (n factors)
  (sort (remove-duplicates
	    (cons n (cl-loop for f in (aref factors n)
			  for subfactors = (all-factors-3 f factors)
			  append (cons f subfactors))))
    #'<))
;;(all-factors-3 20 +factors+)

(cl-defun all-factors-2 (n)
...
(cl-defun find-min-modular-inverses (ns)
  (cl-loop for i in ns
	for x = (find-min-modular-inverse i)
	if (> x 1) collect (list i x)))
;;(length (find-min-modular-inverses (0-n 10000)))

(cl-defun sum-max-modular-inverse (a b)
  (cl-loop for n from a to b sum (find-modular-inverse n)))
...
(cl-defun pvecs-combine (pvec1 pvec2)
  (cl-loop for f1 in pvec1 append
	(cl-loop for f2 in pvec2 collect (* f1 f2))))
...
(cl-defun all-divisors (pfs primes)
  (all-divisors-1 (cl-mapcar #'pvec pfs primes)))
;;(all-divisors '(5 1 2) '(2 3 5))

(cl-defun pvec (pmax prime)
  (cl-loop for i from 1 to pmax collect (expt prime i)))
;;(pvec 5 2)

;; (let ((n (expt 10 7)))
;;   (cl-loop for a from n to (+ n 500)
