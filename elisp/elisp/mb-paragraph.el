...
(defconst time-regexp "[0-2][0-9]:[0-5][0-9]")
(cl-defun kill-time-exp () (kill-word 2))
...
(cl-defun kill-region* (beg end &optional with-properties)
...
(cl-defun kill-current-paragraph ()
  (kill-region* (bop) (eop)))

(cl-defun kill-current-schedule-paragraph ()
  "Kills current paragraph and returns the pair (TIMESTAMP TEXT)"
  (let ((res (split-string* (kill-current-paragraph) time-regexp :right)))
    (if (= (length res) 1)
      (cons nil res) res)))

(cl-defun schedule-fill-paragraph (&optional justify)
...
(cl-defun set-locals-arbeidslog ()
