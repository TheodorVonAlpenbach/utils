...
(cl-defstruct (midi-file (:type list) :named (:conc-name mf-))
  (original-file)
  (header-tag)
  (format-type)
  (time-division)
...
header. Format is 0, 1, 2."
  (with-file-readonly file
    ...
    (let* ((header-tag (bytes-to-string (read-bytes 4)))
	  ...
	  (tracks (cl-loop for i below number-of-tracks
...
(cl-defun write-midi-file (mf file)
...
(cl-defun midi-file-to-bytes (mf)
...
;;; queries
(cl-defun midi-file-ticks-per-beat (mf)
  (mf-time-division mf))

(cl-defun get-midi-meta-events (mt &optional type)
...
(cl-defun test-midi (&optional (n 1)) 
  (read-midi-file (test-midi-filename n)))
;;(test-midi 1)

(cl-defun test-midi-edit-instruments (file nfile)
...
(cl-defun test-midi-stupid-copy (file nfile)
...
(cl-defun test-midi-set-tempo (bpm filename &optional (nfile (file-name-append filename (format "-%d" bpm))))
