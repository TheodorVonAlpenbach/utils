...
(let ((q nil))
  (cl-defun q-test (&optional reset)
    (when (or reset (not q))
      (setq q (list reset ())))
    q))
...
(cl-defun q-eat-regexp (q regexp)
...
(cl-defun q-barf (q &optional (n 1))
  "Reverts the last N operations on Q by `q-eat'
Seems to be obsolete now."
  (cl-loop repeat (min n (length (q-stack q)))
...
(cl-defun lab-dttm (time &optional with-seconds)
...
(defconst *lab-rolling-tokens* 
  '((now "now")
    ...
    (yearstart "yearstart")) )
(cl-defun lab-rolling-token (symbol)
...
(defconst *lab-operator-tokens* 
  '((plus "+")
    ...
    (closed-left "\\[")
    (closed-right "\\]")))
(cl-defun lab-operator-token (symbol)
...
(cl-defun lab-time-point-parse (q)
...
(cl-defun lab-time-point-atom-parse (q)
...
(cl-defun lab-time-point-atom-fixed-parse (q)
...
;;modifiers
(cl-defun lab-time-point-modifiers (q)
...
(cl-defun lab-interval-atom-parse (q)
  "Returns an interval"
  (or 
   (q-try q
...
(cl-defun lab-time-string-to-sql (lp lv rv rp)
...
(cl-defun lab-time-string-to-solr (lp lv rv rp)
  (format "[%s TO %s]"
    (solr-time lv (eq lp 'open-left))
    (solr-time rv (eq rp 'open-right))))

(cl-defun lab-time-string (string &optional (format 'sql))
