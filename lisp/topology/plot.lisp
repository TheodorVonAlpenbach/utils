...
(defmethod format-object ((x point) &optional (n 1))
  ...
  (apply
   #'format nil
   "set label at ~d,~d \"\" point pointtype 6 pointsize 2"
   (coordinates x)))
...
(defmethod format-object ((x segment) &optional (n 1))
  (apply #'format nil
	 "set object ~d polygon from ~d,~d to ~d,~d to ~d,~d fs empty border 1"
	 n (flatten* (list (coordinates x)
			   (coordinates (start x))))))
...
;;(let ((*scale-offset* 0.5)) (plot (make-ellipse-origin 2 1)))
