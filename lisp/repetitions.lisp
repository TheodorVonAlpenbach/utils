...
which is also a prefix of PATTERN. See also `lppattern'."
  ...
  (let* ((m (length pattern))
	 (n (length text))
	 (lptext (make-array (1+ n) :initial-element 0))
	 (k 0)
...
`lptext' for further details. See also comment in the foreword."
  (let* ((m (length pattern))
	 (lppattern (make-array (1+ m) :initial-element 0))
	 (k 1))
...
itself, so the last entry is always M."
  (nreverse (lppattern (reverse pattern) :test test :key key)))
;;(lspattern "abcabc")
...
`lspattern' and `lptext'."
  (nreverse (lptext (reverse pattern) (reverse text) 
		       :lppattern (and lspattern (reverse lspattern))
		       :test test
		       :key key)))
;;(lstext "abq" "cababq")
...
U. Finally the U-OFFSET must be added."
  (let ((m (length u)))
    (offset-center-postitions (right-repetitions (reverse v) (reverse u) 0 min-length test key with-center-repetitions-p)
			      #'(lambda (x) (- m x)))))
...
;;(repetitions "aaaaaaaaaaaaaaaa")

(defun test-repetitions (&rest args)
  (with-open-file (stream "/cygdrive/c/Users/eier/Google Drive/projects/yamal/lisp/utils/repetitions.lisp")
    (let ((string (make-string (file-length stream))))
      (read-sequence string stream)
      (let* ((reps (apply #'repetitions string args))
	     (longest-rep (first (sort reps #'> :key #'first)))
	     (len (first longest-rep))
	     (pos (first (second longest-rep)))
	     (context 3))
	(when reps
	  (subseq string (- pos len context) (+ pos len context)))))))
;||||;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;||||
